    ...
    <div className="modal-backdrop fixed inset-0 flex items-end sm:items-center justify-center z-50 p-4" onClick={handleClose}>
      ...
      >
        ...
        <div className="pt-9 pb-6 px-6 max-h-[85vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl">
          <h2 className="font-display gold-text text-3xl font-extrabold text-center mt-2 mb-1 tracking-wide">Досягнення</h2>
          <p className="text-violet-200 text-center text-sm mb-6">Здобувай зірки й перемоги, щоб відкрити всі нагороди</p>
            ...
            {BADGES.map((b) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className={`text-xs font-semibold leading-tight ${earned ? "text-white" : "text-white/60"}`}>{b.name}</span>
                </div>
              );
            })}
          </div>
